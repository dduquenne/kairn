...
function isProduct(value: unknown): value is Product {
  return typeof value === 'object' && value !== null && 'id' in value && 'price' in value;
}
if (isProduct(data)) {
  /* TypeScript sait que c'est un Product ici */
}
```
...
describe('[Module].[Fonction]', () => {
  ...
  it('rejette les entrees malformees', () => {
    expect(() => myFunction({ id: '<script>alert(1)</script>' })).toThrow('Invalid input');
  });
});
```
...
describe('fetchUserProfile', () => {
  ...
  it('propage l erreur de base de donnees avec le bon message', async () => {
    vi.mocked(prisma.user.findUnique).mockRejectedValue(new Error('Connection timeout'));
