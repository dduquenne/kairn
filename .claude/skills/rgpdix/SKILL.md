...
L'application fournit les briques RGPD suivantes :

```
src/
├── components/
│   ├── legal-notice.tsx        ← Mentions légales configurables
│   ├── privacy-policy.tsx      ← Politique de confidentialité
│   └── cookie-banner.tsx       ← Bandeau cookies (consentement)
├── app/api/rgpd/
│   ├── export/route.ts         ← Droit d'accès / portabilité (JSON/CSV)
│   ├── delete/route.ts         ← Droit à l'effacement
│   └── audit/route.ts          ← Journal des demandes d'exercice de droits
└── config/rgpd.config.ts       ← RGPDConfig (raison sociale, DPO, finalités)
```
...
- Traitement de données RH sensibles → **recommandé**
