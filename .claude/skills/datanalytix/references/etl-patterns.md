...
serve(async req => {
  ...
  const sfData = await fetchExternalRecords(lastSyncDate);
  ...
  const mapped = sfData.map(record => ({
    ...
    status: mapExternalStatus(record.Status),
...
Pour la consolidation multi-source :
...
BEGIN
  -- Traiter les enregistrements importés
