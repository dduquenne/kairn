...
description: >
  ...
  à la consolidation multi-sources (services externes), aux pipelines ETL/ELT, aux
  ...
  "KPI", "indicateur", "tableau de bord analytique",
...
compatibility:
  recommends:
    ...
    - integratix # Pour les connecteurs d'extraction depuis les sources externes
...
| **C — Staging + Transform**            | Multi-source         | Table de staging → fonction de transformation → table production |
...
### Consolidation multi-source (exemple)

```
Source A ──► Edge Function ──► staging_sav ──► process_staging()
Source B ──► Edge Function ──► staging_sav ───────┘
                                                    ▼
                                          sav_tickets (prod)
                                                    ▼
