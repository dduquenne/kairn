...
function handleAnimal(animal: Animal): string {
  switch (animal.type) {
    case 'cat':
      return animal.purrs ? 'purring' : 'silent';
    case 'dog':
      return animal.barks ? 'barking' : 'silent';
    default:
      return assertNever(animal); // Erreur de compilation si cas manquant
  }
}
```
  ...
  return function (this: unknown, ...args: unknown[]) {
    ...
    if (result instanceof Promise) {
      return result.then(r => {
        logger.debug({ method: methodName }, 'Method resolved');
        return r;
      });
    }
    return result;
