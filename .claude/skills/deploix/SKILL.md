...
métier TypeScript/Next.js de l'application Link's Accompagnement hébergées sur Vercel avec Supabase.
...
GitHub (links-app)
    │
    ├── push sur main         ───► Vercel Production deployment
...
Le projet Vercel pointe sur la racine `/` du dépôt. Pas de `ignoreCommand` nécessaire
(projet standalone, pas de monorepo).
...
| ------------------------------- | ----------- | -------------------- | ------------------------------ |
| `NEXT_PUBLIC_SUPABASE_URL`      | Publique    | Preview + Production | `@/lib/supabase`, `@/lib/auth` |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Publique    | Preview + Production | `@/lib/supabase`, `@/lib/auth` |
| `SUPABASE_SERVICE_ROLE_KEY`     | **Secrète** | Production only      | `@/lib/supabase` (serveur)     |
| `RESEND_API_KEY`                | **Secrète** | Production only      | `@/lib/email`                  |
