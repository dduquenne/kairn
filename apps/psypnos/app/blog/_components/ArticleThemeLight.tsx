...
/**
 * ============================================================================
 * COMPOSANT POUR AFFICHER L'ARTICLE EN MODE JOUR (LIGHT)
 * ============================================================================
 *
 * Ce composant applique tous les styles spécifiques au mode jour:
 * - Fond très foncé (night semi-transparent)
 * - Texte clair (ivory)
 * - Accent or adapté (gold adapté au light mode)
 *
 * Pour modifier les couleurs du mode jour, modifiez les valeurs dans:
 * app/blog/_constants/themeConstants.ts > THEME_LIGHT
 */
export function ArticleThemeLight({ content }: ArticleThemeLightProps) {
  const boxStyles = THEME_LIGHT.box;
  const contentVars = THEME_LIGHT.components;
  const typographyVars = THEME_LIGHT.typography;
  const spacingVars = THEME_LIGHT.spacing;
  const otherVars = THEME_LIGHT.other;
    ...
    >
      ...
      <div className="max-w-none mx-auto" style={{ maxWidth: "720px" }}>
        ...
        >
          <style>{`
            /* ============================================================================
               MODE JOUR (LIGHT) - Styles pour le contenu de l'article
               ============================================================================
               Fond: Très foncé (Night semi-transparent)
               Texte: Clair (Ivory)
               Accent: Or adapté (Gold adapté pour light mode)

               Modification: Changez les valeurs dans themeConstants.ts > THEME_LIGHT
            */
