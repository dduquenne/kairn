...
function StatCard({
    ...
    return (
      <div className="border-gold/20 bg-night/60 rounded-xl border p-4">
        <div className="animate-pulse space-y-2">
          <div className="bg-gold/20 h-3 w-20 rounded" />
          <div className="bg-gold/10 h-7 w-16 rounded" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-xl border p-3 sm:p-4 ${accentColors[color]}`}>
      <p className="text-ivory/50 text-[11px] font-medium uppercase tracking-wide sm:text-xs">
        {label}
      </p>
      <p className={`mt-1 text-xl font-bold sm:text-2xl ${valueColors[color]}`}>{value}</p>
      {sub && <p className="text-ivory/40 mt-0.5 truncate text-[11px] sm:text-xs">{sub}</p>}
    </div>
  );
}
