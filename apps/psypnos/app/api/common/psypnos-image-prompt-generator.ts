...
/**
 * Générateur spécialisé de prompts image IA pour PSYPNOS
 * Focus: Infographie transpersonnelle, symbolique, abstraite
 * Style: Contemplative, introspective, esthétiquement cohérente avec Psypnos
 *
 * UTILISATION :
 * - PSYPNOS_IMAGE_GENERATION_PROMPT : System prompt avec directives détaillées
 ...
 * - PSYPNOS_BRAND_COLORS : Palette de couleurs officielle alignée sur la charte graphique
 *
 * EXEMPLES :
...
/**
 * PALETTE DE COULEURS OFFICIELLE PSYPNOS
 * Alignée sur lib/config/theme.ts pour cohérence parfaite
 */
export const PSYPNOS_BRAND_COLORS = {
  // Couleurs primaires de marque
  gold: '#c7a962',           // Or standard Psypnos (principal)
...
3. COHÉRENTES avec l'identité visuelle Psypnos (4 éléments clés)
...
3. ✅ **PALETTE DOMINANTE : OR PSYPNOS + BLEU NUIT**
   - Or Psypnos (#c7a962) comme couleur primaire dominante
...
- ✅ **COHÉRENTES avec Psypnos** (4 éléments clés + palette officielle)

---

## IDENTITÉ VISUELLE PSYPNOS

### Palette de couleurs officielle (RESPECTER STRICTEMENT)
- **Or Psypnos** (#c7a962) : Couleur primaire, illumination, sagesse, transformation
...
- Utiliser les **images/métaphores** pertinentes à la psychothérapie/développement personnel
...
Spécifie 3-4 couleurs dominantes de la palette Psypnos :
...
Palette : Dominante or Psypnos (#c7a962) pour la silhouette et les halos, fond bleu nuit (#0e1f2f) profond et apaisant, ivoire (#f5f1e6) pour la lumière douce, touches de sauge (#7b9d8f) pour l'ancrage.
...
- ✅ Palette officielle : "Or Psypnos (#c7a962)...bleu nuit (#0e1f2f)...ivoire (#f5f1e6)"
...
Palette : Fond bleu nuit (#0e1f2f) profond et sécurisant, or Psypnos dominant (#c7a962), ivoire (#f5f1e6) pour pureté et légèreté, pourpre doux (#9b7eaa) en accents subtils.
...
Palette : Or Psypnos dominant (#c7a962) pour l'irradiation, bleu nuit (#0e1f2f) pour l'espace vaste et profond, terracotta (#cd853f) pour l'ancrage terrestre, ivoire (#f5f1e6) pour le vide fertile lumineux.
...
❌ **Couleurs non-Psypnos** : UTILISER UNIQUEMENT les couleurs de la palette officielle
...
**PRIORITÉ 3 : IDENTITÉ VISUELLE PSYPNOS (4 ÉLÉMENTS CLÉS)**
...
- [ ] Utilise **UNIQUEMENT les couleurs de la palette Psypnos officielle**
 ...
 * Utilise les couleurs officielles Psypnos
 */
export const THEMATIC_VISUAL_MAPPING: Record<string, {
  ...
  anxiety: {
    ...
    colorAccent: "Or Psypnos (#c7a962) dominant et enveloppant, fond bleu nuit (#0e1f2f), touches de sauge (#7b9d8f) pour l'ancrage",
  ...
  depression: {
    ...
    colorAccent: "Bleu nuit (#0e1f2f) en base, or Psypnos (#c7a962) comme lumière d'espoir, ivoire (#f5f1e6) pour la clarté",
  ...
  forgiveness: {
    ...
    colorAccent: "Pourpre (#9b7eaa), or Psypnos (#c7a962) pour l'amour, ivoire (#f5f1e6) pour la pureté",
...
export function enrichImagePromptWithThematics(
  ...
  const keywordMappings: Record<string, string> = {
    ...
    'hypnose': 'hypnose', 'hypnotique': 'hypnose', 'transe': 'hypnose',
...
export function findThematicMatch(topic: string): {
  ...
  const keywordMappings: Record<string, string> = {
    ...
    'hypnose': 'hypnose', 'respiration': 'respiration',
 ...
 * Version 2.0 - Utilise les couleurs officielles Psypnos et vérifie la chaleur/accueil
...
export function validatePromptForMandatoryElements(prompt: string): {
  ...
  // 3. Vérifier la palette officielle Psypnos (or + bleu nuit)
  ...
  if (!hasPaletteOB) missingElements.push('Palette or Psypnos (#c7a962) + bleu nuit (#0e1f2f)');
  ...
  if (!hasPaletteOB) {
    suggestedCorrections.push('Spécifier la palette Psypnos officielle (ex: "Palette : or Psypnos (#c7a962), bleu nuit (#0e1f2f), ivoire (#f5f1e6)")');
  }
