...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (post.status === 'PUBLISHING') {
      return NextResponse.json({ error: 'Ce post est en cours de publication' }, { status: 400 });
    }

    // Récupérer le compte avec les tokens
    const account = await getSocialAccountById(post.accountId);
    if (!account) {
      return NextResponse.json({ error: 'Compte social non trouvé' }, { status: 404 });
    }

    if (!account.isActive) {
      return NextResponse.json({ error: 'Le compte social est désactivé' }, { status: 400 });
    }
    ...
    try {
      ...
      if (result.success && result.externalPostId) {
        ...
        console.log(
          `[Social Publish] Published post ${id} to ${post.platform} -> ${result.externalPostId} (${result.platformUrl || 'no url'})`
        );
