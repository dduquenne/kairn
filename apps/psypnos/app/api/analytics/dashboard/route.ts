...
import { isMockMode, generateMockDashboardData, generateMockGeolocationData, logDataMode } from "@/lib/pwaDataMode";
...
async function fetchGeoData(startDate: Date, endDate: Date) {
  try {
    ...
    const cityStats: Record<string, { count: number; country: string; countryCode: string; region: string | null; latitude: number | null; longitude: number | null }> = {};
    ...
    for (const geo of geolocations) {
      ...
      if (geo.city) {
        ...
        if (!cityStats[cityKey]) {
          cityStats[cityKey] = { count: 0, country: geo.country, countryCode: geo.countryCode, region: geo.region, latitude: geo.latitude, longitude: geo.longitude };
        }
    ...
    const byCity = Object.entries(cityStats)
      .map(([key, data]) => {
        ...
        return { city, country: data.country, countryCode: data.countryCode, region: data.region, latitude: data.latitude, longitude: data.longitude, visitors: data.count };
      })
      .sort((a, b) => b.visitors - a.visitors);
...
async function fetchBlogAnalytics(startDate: Date, endDate: Date) {
  try {
    const allAnalytics = await prisma.blogAnalytics.findMany({
      ...
      select: { articleSlug: true, sessionId: true, timestamp: true, scrollDepthPercent: true, timeOnPage: true, completed: true },
    });

    const grouped = new Map<string, { views: number; sessions: Set<string>; lastViewed: Date; scrollDepths: number[]; timesOnPage: number[]; completedReads: number }>();

    for (const r of allAnalytics) {
      let entry = grouped.get(r.articleSlug);
      if (!entry) {
        entry = { views: 0, sessions: new Set(), lastViewed: r.timestamp, scrollDepths: [], timesOnPage: [], completedReads: 0 };
    ...
    const articles = Array.from(grouped.entries()).map(([slug, d]) => ({
      ...
      engagement: {
        avgScrollDepth: d.scrollDepths.length > 0 ? Math.round(d.scrollDepths.reduce((a, b) => a + b, 0) / d.scrollDepths.length) : null,
        avgTimeOnPage: d.timesOnPage.length > 0 ? Math.round(d.timesOnPage.reduce((a, b) => a + b, 0) / d.timesOnPage.length) : null,
      },
      score: 0,
    }));
