...
export const THREADS_FORMATS: Record<ThreadsPostFormat, ThreadsFormatSpec> = {
  pensee_brute: {
    ...
    structure: ['Une pensée directe (1-2 phrases)', 'Optionnel: une nuance ou extension courte'],
    examples: [
    ...
    bestFor: [
      ...
      'Insights sur la condition humaine',
      'Observations universelles',
    ],
    tips: [
  ...
  observation_cabinet: {
    ...
    bestFor: [
      ...
      "Partager de l'expertise sans être didactique",
    ],
    tips: [
      'Toujours anonymiser',
  ...
  micro_confession: {
    ...
    structure: ['Admission personnelle', 'Normalisation ou réassurance'],
    examples: [
  ...
  fragment_poetique: {
    ...
    structure: ['Une image ou sensation évoquée', 'Optionnel: une résonance ou silence'],
    examples: [
      'Respirer. Juste ça. Le reste peut attendre.',
      "Ce moment suspendu où tout devient possible. L'hypnose, c'est ça.",
  ...
  contre_intuitif: {
    ...
    examples: [
      "L'hypnose, ce n'est pas dormir. C'est se réveiller autrement.",
...
export const THREADS_AUTHENTICITY_LEVELS: Record<
  ThreadsAuthenticityLevel,
  ThreadsAuthenticitySpec
> = {
  1: {
    level: 1,
  ...
  5: {
    ...
    characteristics: ['Parle comme à un ami proche', "N'édulcore pas", 'Assume ses imperfections'],
...
export const THREADS_HOOK_PATTERNS: ThreadsHookPattern[] = [
  ...
  },
  {
    ...
    examples: ['Respirer. Juste ça.', "S'arrêter. Enfin.", 'Écouter. Sans chercher à répondre.'],
    bestFor: ['inspirant'],
  },
  {
    id: 'paradoxe',
    ...
    examples: [
      "L'hypnose, ce n'est pas dormir. C'est se réveiller.",
 ...
 */
export function suggestThreadsFormat(tone: ContentTone, angle: ContentAngle): ThreadsPostFormat {
...
export function validateThreadsContent(content: string): {
  ...
  if (content.length > 500) {
    ...
    warnings.push(
      `Le contenu fait ${content.length} caractères. Idéalement moins de ${THREADS_RULES.maxLength}.`
    );
  }

  if (content.length < THREADS_RULES.minLength) {
    warnings.push(
      `Le contenu est très court (${content.length} caractères). Minimum suggéré: ${THREADS_RULES.minLength}.`
    );
  }
