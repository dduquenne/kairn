...
const PSYPNOS_CONTEXT = `Tu es un expert en communication digitale pour un cabinet de psychothérapie.
...
export function buildSystemPrompt(): string {
  return `${PSYPNOS_CONTEXT}

MISSION:
Tu vas générer du contenu pour les réseaux sociaux de Psypnos à partir d'articles de blog.
...
4. Renforcent l'image de marque Psypnos
...

{
  ...
  "hashtags": ["${suggestedHashtags[0] || 'hypnose'}", "${suggestedHashtags[1] || 'bienetre'}"],
...
• 1 hashtag NICHE : ciblage précis (ex: #hypnoseericksonienne)
...

{
  ...
  "hashtags": ["${suggestedHashtags[0] || 'bienetre'}", "${suggestedHashtags[1] || 'santementale'}", "${suggestedHashtags[2] || 'psychotherapie'}", "${suggestedHashtags[3] || 'hypnose'}"],
  "suggestedMediaAlt": "Description alternative pour l'image si une est utilisée",
  "formatUsed": "${format}",
  "hookPatternUsed": "nom du pattern utilisé"
}
