...
const STORAGE_KEY = 'psypnos_navigation_history';
const CONSENT_KEY = 'psypnos_prefill_consent';
...
> = {
  ...
  '/services/hypnose': { service: 'hypnose', keywords: ['hypnose'], weight: 2 },
...
const SUGGESTED_MESSAGES: Record<string, string> = {
  ...
  tabac: "Bonjour, je suis intéressé(e) par l'arrêt du tabac par hypnose.",
  ...
  hypnose: "Bonjour, je suis intéressé(e) par l'hypnothérapie.",
...
function getSessionId(): string {
  ...
  let sessionId = sessionStorage.getItem('psypnos_session_id');
  if (!sessionId) {
    sessionId = `session-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    sessionStorage.setItem('psypnos_session_id', sessionId);
  }
  return sessionId;
}
