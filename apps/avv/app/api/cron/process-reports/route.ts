  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 700px; margin: 0 auto; padding: 20px;">
      <div style="background-color: #1a1a2e; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: #D4AF37; margin: 0; font-size: 24px;">${frequencyEmoji} Appréciez Votre Vie Analytics</h1>
      ...
      <div style="background-color: #f8f9fa; padding: 25px; border-radius: 0 0 10px 10px;">
        ...
        <div style="text-align: center; padding-top: 20px; border-top: 1px solid #ddd;">
          <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://appreciezvotrevie.fr'}/admin/analytics"
          ...
          <p style="color: #888; font-size: 12px; margin-top: 20px;">
            Ce rapport est généré automatiquement par Appréciez Votre Vie Analytics.
          </p>
        </div>
      </div>
    </body>
...
async function sendReportEmail(recipients: string[], subject: string, html: string): Promise<void> {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.REPORT_EMAIL_FROM || 'Appréciez Votre Vie Analytics <analytics@appreciezvotrevie.fr>',
      to: recipients,
      subject: `[Appréciez Votre Vie] ${subject}`,
      html,
    }),
  });
