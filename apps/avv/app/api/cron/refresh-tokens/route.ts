...
async function notifyAccountsNeedingReconnection(
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.ALERT_EMAIL_FROM || 'Appréciez Votre Vie <notifications@appreciezvotrevie.fr>',
        to: [adminEmail],
        subject: '⚠️ Appréciez Votre Vie - Comptes sociaux nécessitant reconnexion',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="margin-top: 20px;">
              <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://appreciezvotrevie.fr'}/admin/social/accounts"
                 style="background: #3498db; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">
                Gérer les comptes
              </a>
            </p>
            <p style="color: #666; font-size: 12px; margin-top: 30px;">
              Ce message est envoyé automatiquement par le système de maintenance de Appréciez Votre Vie.
            </p>
          </div>
        `,
      }),
