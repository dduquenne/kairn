...
// Navigation configuration for Appréciez Votre Vie admin
const avvAdminNavigation: NavigationItem[] = [
...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="bg-night/95 text-ivory flex min-h-screen">
      <AdminSidebar navigation={avvAdminNavigation} siteName="Appréciez Votre Vie" />
        ...
        <header className="sticky top-0 z-40 border-night/50 bg-night/80 border-b px-4 py-3 backdrop-blur sm:px-6 sm:py-4">
          <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <MobileNav navigation={avvAdminNavigation} siteName="Appréciez Votre Vie" />
              <div>
                <p className="text-gold text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em]">
                  Tableau de bord
                </p>
                <h1 className="text-lg font-semibold sm:text-2xl">Administration Appréciez Votre Vie</h1>
              </div>
            </div>
