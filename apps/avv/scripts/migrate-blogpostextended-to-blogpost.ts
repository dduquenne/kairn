...
// Appréciez Votre Vie site ID (from database)
const AVV_SITE_ID = 'cmkpjzwu00000zc986w1kta2y';
...
async function getOrCreateTag(tagName: string): Promise<string> {
  ...
  const existingTag = await prisma.tag.findUnique({
    where: { slug_siteId: { slug, siteId: AVV_SITE_ID } },
  });
...
async function main() {
  ...
  const site = await prisma.site.findUnique({
    where: { id: AVV_SITE_ID },
  });

  if (!site) {
    console.error(`❌ Site not found: ${AVV_SITE_ID}`);
    process.exit(1);
  }
  ...
  console.log(`\n📊 Total BlogPost articles for Appréciez Votre Vie: ${finalCount}`);
