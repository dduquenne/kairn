...
/**
 * Migration script: Import blog articles from AVV to KAIRN
 *
 * This script supports multiple data sources:
 * 1. Direct PostgreSQL connection to AVV (AVV_DATABASE_URL)
 * 2. Supabase REST API (AVV_SUPABASE_URL + AVV_SUPABASE_SERVICE_KEY)
 *
 * Run with: npx tsx apps/avv/scripts/migrate-blog-from-avv.ts
 *
 ...
 *
 * Required env variables for AVV (source) - ONE of these:
 * - AVV_DATABASE_URL: PostgreSQL connection string for AVV
 *   OR
 * - AVV_SUPABASE_URL + AVV_SUPABASE_SERVICE_KEY: Supabase REST API credentials
 */
...
// AVV Supabase defaults (from MCP config)
const AVV_PROJECT_REF = 'ukbbkoadbgifnxbcuxbr';
const AVV_DEFAULT_URL = `https://${AVV_PROJECT_REF}.supabase.co`;
...
function initAvvConnection(): DataSource {
  ...
  if (process.env.AVV_DATABASE_URL) {
    avvPool = new Pool({
      connectionString: process.env.AVV_DATABASE_URL,
      ssl: { rejectUnauthorized: false },
    });
    console.log('📦 Source AVV: PostgreSQL direct connection');
    return 'postgres';
  }

  // Try Supabase REST API
  const supabaseUrl = process.env.AVV_SUPABASE_URL || AVV_DEFAULT_URL;
  const supabaseKey = process.env.AVV_SUPABASE_SERVICE_KEY;

  if (supabaseKey) {
    avvSupabase = createClient(supabaseUrl, supabaseKey, {
      auth: { autoRefreshToken: false, persistSession: false },
    });
    console.log('📦 Source AVV: Supabase REST API');
    return 'supabase-api';
  }

  console.error('❌ No AVV data source configured!');
  console.error('   Set one of:');
  console.error('   - AVV_DATABASE_URL (PostgreSQL connection string)');
  console.error('   - AVV_SUPABASE_SERVICE_KEY (with optional AVV_SUPABASE_URL)');
  process.exit(1);
}
...
async function fetchAvvArticlesSupabase(): Promise<AvvBlogPost[]> {
  ...
  const { data, error } = await avvSupabase
    .from('BlogPostExtended')
    .select('*')
