...
export function GlobalHeader({ context = 'home', showBackButton = false }: GlobalHeaderProps) {
  ...
  const getNavText = (): { show: boolean; text?: string; href: string; icon?: boolean } => {
    switch (context) {
      ...
      case 'blog-list':
        return { show: true, text: 'Appréciez Votre Vie', href: '/', icon: false };
      case 'appointment':
      case 'seminar':
      case 'privacy':
        return { show: true, text: 'Appréciez Votre Vie', href: '/', icon: false };
          ...
          >
            <div>
              <img
                ...
                alt="Nathalie Duquenne - Sophrologie et Somatothérapie"
                className="fade-mask h-24 w-24"
              />
              <h2 className="text-gold text-sm font-medium">Nathalie Duquenne</h2>
            </div>
