...
import type { LinkedInPostFormat, LinkedInExpertiseLevel, ContentTone, ContentAngle } from '../types';
...
#anxiete #santementale #psychotherapie #bienetre`,
    bestFor: ['Déconstruction de mythes', 'Changement de perspective', 'Positionnement différenciant'],
    tips: [
...
En somatothérapie, on ne cherche pas à "vider la tête".
...
#somatothérapie #stress #dirigeants #bienetre #santementale`,
...
export const LINKEDIN_HOOK_PATTERNS: LinkedInHookPattern[] = [
  ...
  },
  {
    ...
    examples: [
      'Pendant 10 ans, j\'ai cru que l\'somatothérapie ne fonctionnait que sur certaines personnes.',
  ...
  },
  {
    ...
    examples: [
      ...
      'Ce qu\'on ne vous dit pas sur l\'somatothérapie.',
...
export const LINKEDIN_EXPERTISE_LEVELS: Record<LinkedInExpertiseLevel, LinkedInExpertiseSpec> = {
  ...
  2: {
    ...
    exampleOpening: 'Je remarque souvent que les personnes anxieuses partagent cette caractéristique...',
 ...
 */
export function suggestLinkedInFormat(
  tone: ContentTone,
  angle: ContentAngle
): LinkedInPostFormat {
  ...
  for (const category of largeCategories) {
    ...
    for (const tag of category.hashtags) {
      if (!result.includes(tag) && result.length < count) {
        ...
        if (
          (themeLower.includes('somatothérapie') && tag === 'somatothérapie') ||
