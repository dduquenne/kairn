...
describe('Auth Flow Integration', () => {
  ...
  describe('Login Flow', () => {
    ...
    it('should normalize email to lowercase', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.user?.email).toBe('admin@appreciezvotrevie.fr');
    });

    it('should clear rate limit attempts on successful login', async () => {
      // First, trigger some failed attempts
      await handleLogin({ email: 'admin@appreciezvotrevie.fr', password: 'wrong1' });
      await handleLogin({ email: 'admin@appreciezvotrevie.fr', password: 'wrong2' });

      // Check that attempts were recorded
      expect(mockRateLimitStore.get('admin@appreciezvotrevie.fr')).toBe(2);
      ...
      expect(mockRateLimitStore.get('admin@appreciezvotrevie.fr')).toBeUndefined();
    });
  });
  ...
  describe('Rate Limiting', () => {
    ...
    it('should record failed attempt on invalid credentials', async () => {
      ...
      expect(mockRateLimitStore.get('admin@appreciezvotrevie.fr')).toBe(1);
    });
  ...
  describe('Token Verification', () => {
    it('should verify valid token', async () => {
      ...
      expect(payload?.email).toBe('admin@appreciezvotrevie.fr');
  ...
  describe('Complete Flow', () => {
    it('should complete full login and protected access flow', async () => {
      ...
      expect(authorizedResult.user?.email).toBe('admin@appreciezvotrevie.fr');
    });
  });
});
