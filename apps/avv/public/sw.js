...
/**
 * Appréciez Votre Vie Service Worker - PWA Avancée
 *
 * Fonctionnalités:
...
const CACHE_NAME = `avv-${CACHE_VERSION}`;
const STATIC_CACHE = `avv-static-${CACHE_VERSION}`;
const DYNAMIC_CACHE = `avv-dynamic-${CACHE_VERSION}`;
...
const SYNC_QUEUE_KEY = 'avv_sync_queue';
...
async function getOfflinePage() {
  ...
  return new Response(
    ...
    <head>
      ...
      <title>Hors ligne - Appréciez Votre Vie</title>
      <style>
...
async function queueForSync(request) {
  ...
  if ('sync' in self.registration) {
    try {
      await self.registration.sync.register('avv-sync');
