/**
 * Shared Email Templates for Kairn
 *
...
/**
 * Convert newlines to HTML line breaks with XSS escaping
 */
...
/**
 * Format a submission timestamp (ISO string) to a French date
 */
...
/**
 * A key-value field in an email section
 */
...
/**
 * A titled section containing key-value fields
 */
...
/**
 * A highlighted callout box in confirmation emails
 */
...
/**
 * Options for building an admin notification email
 */
...
/**
 * Options for building a user confirmation email
 */
...
/**
 * Site branding information for email templates
 */
export interface EmailBranding {
  ...
  domain: string;
  /** Short tagline (e.g. "Psychothérapie · Hypnose") */
  tagline: string;
  ...
  colors: {
    ...
    secondary: string;
    /** Light foreground / text-on-dark color */
    foreground: string;
  };
}
...
/**
 * Build an HTML admin notification email.
 *
 * @param options - Email content options
 * @param branding - Site branding information
...
export function buildAdminEmailHtml(options: AdminEmailOptions, branding: EmailBranding): string {
  ...
  const { heading, badge, sections, messageBlock, preResponse, metadata, submittedAt, sourcePage } =
    options;
...
/**
 * Build a plain text admin notification email
 */
...
/**
 * Build an HTML user confirmation email.
 *
 * @param options - Confirmation email content options
 * @param branding - Site branding information
 * @returns Complete HTML email string
 */
export function buildConfirmationEmailHtml(
  ...
  const c = resolveColors(branding);
  const { recipientName, intro, sections, callout, bulletList, recap, closing, signer, reference } =
    options;
  ...
  const calloutHtml = callout
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;">` +
      ...
      callout.lines
        .map(
          line =>
            `<p style="margin:4px 0;font-size:14px;color:${c.grey700};">${escapeHtml(line)}</p>`
        )
        .join('') +
      `</td></tr></table>`
    : '';
...
/**
 * Build a plain text user confirmation email
 */
export function buildConfirmationEmailText(
  ...
  lines.push(
    ...
    '',
    '─────────────────────',
    `${branding.siteName} · ${branding.domain}`,
    branding.address
  );
