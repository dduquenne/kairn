...
/** Single stat card with accent color */
function StatCard({
  label,
  value,
  sub,
  ...
  return (
    <div className={`rounded-xl border p-4 ${accentColors[color]}`}>
      <p className="text-ivory/50 text-xs font-medium uppercase tracking-wide">{label}</p>
      <p className={`mt-1 text-2xl font-bold ${valueColors[color]}`}>{value}</p>
      {sub && <p className="text-ivory/40 mt-0.5 text-xs">{sub}</p>}
    </div>
  );
}

/** Dashboard stat cards for chatbot overview */
